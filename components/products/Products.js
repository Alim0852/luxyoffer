...
export default function Products(props) {
  ...
  const limit = Number(props?.max) || 3; // default show 3
  const cols = Number(props?.cols) || 4; // default 4 columns (home)
  const gridColsClass = cols === 3 ? 'md:grid-cols-3' : 'md:grid-cols-4';

  return (
    <div className={`productsWrapper grid grid-cols-2 ${gridColsClass} gap-4`}>
      {Product.slice(0, limit).map((item, index) => (
        <div
          key={index}
          className="rounded-md productWrap overflow-hidden productBox flex flex-col mt-10 group cursor-pointer"
          onClick={() => router.push(`/product/${item.slug}`)}
        >
          <div className="relative productImage w-full h-80 bg-gray-50 text-center flex justify-center items-center">
            <Image src={item.image} width={1000} height={1000} alt={item.title} className="w-auto h-72 object-contain" />
            <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 addToCartWrapper absolute inset-0 bg-black/20 flex justify-end pointer-events-none group-hover:pointer-events-auto">
              <div className="actionWrapper flex flex-col gap-2 mt-2 mr-2">
                <button onClick={(e) => { e.stopPropagation(); handleShowToast(); }}>
              ...
              <div className="cartButtonWrapper">
                <button
                  ...
                  className="absolute z-50 bottom-4 left-4 right-4 bg-white rounded-md p-3 text-center font-medium shadow-md"
                >
                  Add To Cart
                </button>
              </div>
