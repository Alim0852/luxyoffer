...
export default function Products(props) {
  ...
  const limit = Number(props?.max) || 3;
  const cols = Number(props?.cols) || 4;
  const gridColsClass = cols === 3 ? 'md:grid-cols-3' : 'md:grid-cols-4';

  return (
    <div className={`productsWrapper grid grid-cols-2 ${gridColsClass} gap-1.5 md:gap-4`}>
      {Product.slice(0, limit).map((item, index) => (
        <div
          key={index}
          className="rounded-md productWrap overflow-hidden productBox flex flex-col mt-6 md:mt-8 lg:mt-10 group cursor-pointer"
        ...
        >
          <div className="relative productImage w-full h-80 bg-gray-50 text-center flex justify-center items-center">
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 addToCartWrapper absolute inset-0 bg-black/20 flex justify-end pointer-events-none group-hover:pointer-events-auto">
              ...
              <div className="cartButtonWrapper">
                <button
                  onClick={(e) => { e.stopPropagation(); handleShowToast(); }}
          ...
          <div className="productMeta">
            <Link href="#" className="flex flex-col gap-2 mt-2" onClick={(e) => e.preventDefault()}>
